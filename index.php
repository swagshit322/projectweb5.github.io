<?php
/**
 * Задание 5: регистрационная форма с авторизацией и редактированием.
 * Поддерживает как неавторизованных (Cookies), так и авторизованных (сессии).
 */
...
function validateLanguages($languages, $pdo) {
    ...
    $invalid = array_filter($languages, fn($l) => !preg_match('/^[a-zA-Z\+\#]+$/', $l));
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    foreach ($fields as $field) {
        ...
        if ($errors[$field] && !empty($_COOKIE[$field . '_error_msg'])) {
            ...
            // не удаляем value – они понадобятся для отображения
        }
    }
